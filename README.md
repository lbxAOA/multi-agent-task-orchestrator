...
- 每个实际使用 A2A peer 的任务都会自动生成一份复盘记录，积累成后续 Skill 优化的证据。
...
对每个实际使用 A2A peer 的团队任务，Hermes 在完成、阻塞或取消时都会从模板创建：

```text
docs/retrospectives/YYYY-MM-DD-<简短-kebab-case-任务名>.md
```

同名任务会自动追加序号，历史记录不会被覆盖。复盘只应包含真实可验证的任务契约、A2A `context_id`、变更范围、实际命令、退出状态和风险；没有运行的测试必须明确标记为未运行及原因。
